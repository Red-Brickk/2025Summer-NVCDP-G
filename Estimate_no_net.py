...
from utils_Estimate_no_net import Sieve_to_coeff, ConstructX4Smodel, ConstructX4Ymodel, calculateSigma_with_sieve, SignificanceTest
...
def Iter_model(Timevec, S0mat, Stensor, Wmat, Ztensor, Ytensor, Atensor, Abar, Gnum=3, J0=2, Gvec_real=None,
    ...
    Sigma_SXZ4S = np.zeros((NN, PP, NN * (J0 + 2), QQ * (J0 + 2)))
    tilde_x4S = np.zeros((NN, TT, PP, QQ*(J0+2), 1))
    ...
    if IV_type == "z" or IV_type == "y":
        ...
        tilde_x4S = tilde_x4S.reshape((NN, TT, PP, QQ * (J0 + 2), 1))
    ...
    Theta_hat4S = np.zeros((Gnum, PP, QQ * (J0 + 4 - 2), 1))
    coeff_hat4S = np.zeros((Gnum, TT, PP, QQ))
