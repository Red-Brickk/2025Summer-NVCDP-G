    ...
    for tt in range(TT):
        for pp in range(PP):
            temp_inv[:, tt, pp, :] = np.linalg.inv(
                np.eye(NN) - np.diag(D1g_hat[:, tt, pp][Gvec_hat].flatten()) @ Wmat
            )
    indexes = np.arange(QQ1) != 0
    temp_mat_mult = np.einsum("ntpq, ntpq -> ntp", features, coeff_hat_by_ind[:,:,:, indexes])[:,:,:,np.newaxis]
